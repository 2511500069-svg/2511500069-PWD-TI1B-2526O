# pertemuan-07

NIM: 2511500069<br>
Nama: Putri S. N. Tampubolon <br>
<br><br>
Hari ini, Kamis 07.Nov.2025 saya belajar:<br>
<ol>
  <li>copy seluruh file dari elearning (punya dosen) umtuk latihan pertemuan-07</li>
  <li>copy dan rename index.php menjadi get.php, kemudain buat file baru get_proses.php utk action</li>
  <li>isi get_proses.php</li>
  <li>session di get_proses.php</li>
  <li>menampilkan session dari get_proses.php ke get.php</li>
  <li>redirect dari get_proses.php ke get.php</li>
  <li>copy paste get.php dan get_proses.php menjadi post.php dan post_proses.php</li>
  <li>sesuaikan get ke post</li>
  <li>session_destroy</li>
  <li>isset</li>
  </ol>
  